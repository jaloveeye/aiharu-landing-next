...
- Next.js 16, React 19, Tailwind CSS 4 기반
...
- 전체 검증: `npm run ci:check && npm run build`

내부 생성·수집·관리 API를 실행하려면 `INTERNAL_API_SECRET`이 필요합니다. 운영 예약 작업과 RLS 적용 절차는 [docs/operations.md](docs/operations.md)를 참고하세요.
...
- **자동 실행**: 직접 운영 서버가 매일 오후 1시(한국시간)에 보호된 API 호출
- **수동 점검**: GitHub Actions 탭에서 뉴스 수집 스크립트 수동 실행 가능
...
운영 설정은 [docs/operations.md](docs/operations.md), 수동 점검은 [docs/github-actions-setup.md](docs/github-actions-setup.md)를 참고하세요.
